...
class Lifecycle extends EventDispatcher, implements ILifecycle {
    ...
    /* Private Properties                                                         */	
    /*============================================================================*/	
    var _reversedEventTypes : Dictionary;
    ...
    /* Public Functions                                                           */	
    /*============================================================================*/	
    public function initialize(callBack : Dynamic->Dynamic = null) : Void {
		_initialize.enter(callBack);
	}

	public function suspend(callBack : Dynamic->Dynamic = null) : Void {
		_suspend.enter(callBack);
	}

	public function resume(callBack : Dynamic->Dynamic = null) : Void {
		_resume.enter(callBack);
	}

	public function destroy(callBack : Dynamic->Dynamic = null) : Void {
		_destroy.enter(callBack);
	}

	public function beforeInitializing(handler : Dynamic->Dynamic) : ILifecycle {
		_initialize.addBeforeHandler(handler);
		return this;
	}

	public function beforeSuspending(handler : Dynamic->Dynamic) : ILifecycle {
		_suspend.addBeforeHandler(handler);
		return this;
	}

	public function beforeResuming(handler : Dynamic->Dynamic) : ILifecycle {
		_resume.addBeforeHandler(handler);
		return this;
	}

	public function beforeDestroying(handler : Dynamic->Dynamic) : ILifecycle {
		_destroy.addBeforeHandler(handler);
		return this;
	}

	public function whenInitializing(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.INITIALIZE, createLifecycleListener(handler, true));
		return this;
	}

	public function whenSuspending(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.SUSPEND, createLifecycleListener(handler));
		return this;
	}

	public function whenResuming(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.RESUME, createLifecycleListener(handler));
		return this;
	}

	public function whenDestroying(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.DESTROY, createLifecycleListener(handler, true));
		return this;
	}

	public function afterInitializing(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.POST_INITIALIZE, createLifecycleListener(handler, true));
		return this;
	}

	public function afterSuspending(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.POST_SUSPEND, createLifecycleListener(handler));
		return this;
	}

	public function afterResuming(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.POST_RESUME, createLifecycleListener(handler));
		return this;
	}

	public function afterDestroying(handler : Dynamic->Dynamic) : ILifecycle {
		addEventListener(LifecycleEvent.POST_DESTROY, createLifecycleListener(handler, true));
		return this;
	}

	override public function addEventListener(type : String, listener : Dynamic->Dynamic, useCapture : Bool = false, priority : Int = 0, useWeakReference : Bool = false) : Void {
